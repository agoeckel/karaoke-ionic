karaoke.controller('PlaylistsCtrl', ['$scope', '$http', '$state','$window', function($scope, $http, $state, $window) {
  ...
  $scope.songInfo = function(){
    ...
    .then(function(response){
      angular.element("#user-playlist").append("<div class='item card list-items'><div id='song-list-headers'><p class='song-list-p'><strong>"+response.config.data.artist+"</strong></p><p class='song-list-p'>"+response.config.data.title+"</p></div><i id="+response.data.song_id+" ng-click='destroySong()' class='icon ion-minus-circled ådelete-song-btn delete-song-btn'><img id='home-album-image' src="+response.data.image_src+" style='float:left; height: 100px; width: 100px'></div>");
      $scope.show = false
